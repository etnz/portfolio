...
`, EnvPortfolioPath, EnvPortfolioPath, EnvDefaultCurrency, EnvDefaultCurrency, EnvVerbose, EnvVerbose)
	...
	args := []string{
		...
		"--ext-flag=val1", // Flag with value for extension
		"--ext-flag2",     // Flag for extension
		"val2",            // Value for the flag
		"positional-arg",  // Positional argument for extension
	}
	...
	// The first arg to the extension is the subcommand name itself.
	expectedArgs := []string{
		"--ext-flag=val1",
		"--ext-flag2",
		"val2",
	...
	for i, arg := range expectedArgs {
		expectedLine := fmt.Sprintf("arg[%d]=%s", i+1, arg)
		if !strings.Contains(output, expectedLine) {
			t.Errorf("Expected output to contain arg %q, but got:\n%s", expectedLine, output)
		}
	}
